...
public class FloatingHUD : MonoBehaviour
{
    ...
    void Update()
    {
        transform.position = Vector3.Lerp(transform.position, scoreTarget.position + new Vector3(0, offSet, 0), .08f); // Move this object a little closer to its target every frame
